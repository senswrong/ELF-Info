...
fun main(args: Array<String>) {
    ...
    var section4String: StringTable? = null
    var dyn4String: StringTable? = null
    elf4File.apply {
        sectionHeaders.forEachIndexed { index, sectionHeader ->
            val name = stringTable.getText(sectionHeader.sh_name)
            when (name) {
                DYNSTR -> dyn4String = tables[index] as StringTable
                SHSTRTAB -> section4String = tables[index] as StringTable
            }
        }
    }
    ...
    elf16File.apply {
        sectionHeaders.forEachIndexed { index, sectionHeader ->
            val name = stringTable.getText(sectionHeader.sh_name)
            when (name) {
                DYNSTR -> dyn16String = tables[index] as StringTable
                SHSTRTAB -> section16String = tables[index] as StringTable
            }
        }
    }

    elf16File.tables.forEachIndexed { index, table16 ->
        val table4 = elf4File.tables[index]
        if (table16 is BaseDataTable && table4 is BaseDataTable) {
            if (!table4.data.contentEquals(table16.data)) {
...
//                                println("$index\t${rela4.r_info.toString(16)} : " +
